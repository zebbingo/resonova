...
| **ASR 处理** | 通过 MQTT 发 chunks，等待 bot 处理 | 同 | 逻辑一致，但当前 chat bot **不响应** |
...
2. **切换为 EOS 触发模式（临时绕过）** — 设置 `MQTT_HANDLE_VAD_ON_SERVER=false`，让 EOS 消息直接触发 STT（同时 `MQTT_SHOULD_SEND_RESPONSE_CHUNK=true` 强制分发响应）
...
全链路测试**未通过**。测试脚本本身工作正常（API 调用全部返回 200），但模拟执行层有两个阻断性问题：

1. **竞态条件**导致大部分模拟 session 不发音频（total_chunks=0）
2. **chatbot 不处理已发送的音频**（所有 audio 都发到 MQTT 了但无响应）

在修复这两个问题之前，"打通全链路"的目标无法达成。

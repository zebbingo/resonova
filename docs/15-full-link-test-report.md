...
设置 `MQTT_HANDLE_VAD_ON_SERVER=false`（EOS 模式，绕过 VAD）后重新测试：
...
**结论：管道本身完好。** VAD 模式对模拟音频不触发是唯一阻断原因。使用 `MQTT_HANDLE_VAD_ON_SERVER=false` 绕过 VAD 后，全链路 STT→LLM→TTS 完全正常。
...
2. **VAD 默认模式不适用于模拟** — 需在 start-local-dev.sh 或测试脚本中处理
